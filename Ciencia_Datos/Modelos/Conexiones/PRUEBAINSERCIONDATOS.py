...
Variables = pd.read_excel(r"D:\Rayogas\Sistemas de Informacion - Estrategia de Datos\7. Fuentes de Informacion\Xls\Comercial\Presupuesto\Formato Meta Comercial.xlsx") 
Variables
...
# Creamos la tabla 
cursor.execute("CREATE TABLE CiudadesPruebasV3(Fecha DATE, U.N. VARCHAR(10), CeCo float, CeCo descripcion VARCHAR(50), CeCo descripcion Ajustado VARCHAR(50), Cedula int, Nombre vendedor VARCHAR(50), Kilos float, Margen_Primo float , MP_Millones float, PRIMARY KEY (Fecha))")


# Insertamos los datos en la tabla Ciudad y nuestra lista variable
cursor.executemany("INSERT INTO CiudadesPruebasV3 VALUES(?,?,?,?,?,?,?,?,?,?)", VariablesLista)
